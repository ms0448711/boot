...
def logo():
 ...
 Zombi Bot V13    |   Ultra Priv8 Bot    | |  Code by Viper1337                           
...
def laravel(url):
    try:
        path0 = se.get(url, headers=Agent, verify=False, timeout=50)
        path6 = se.get(url + '/vendor/phpunit/phpunit/phpunit.xml', headers=Agent, verify=False, timeout=50)
        if path6.status_code == 200 :
            if '/phpunit/phpunit' in path6.content.encode('utf-8'):
                print(ktn4blue + '#[-LARAVEL CMS-]=========> ' + '(' + url + ')' + CEND)
                open('laravel.txt', 'a').write(url + '\n')
        elif '/vendor/phpunit/phpunit/phpunit.xml' in path0.content.encode('utf-8'):
            print(ktn4blue + '#[+LARAVEL CMS+]=========> ' + '(' + url + ')' + CEND)
            open('laravel.txt', 'a').write(url + '\n')
        else:
            print(ktnred + '#[-NOT LARAVEL-]=========> ' + '(' + url + ')' + CEND)
    except:
        pass
    pass
def checkrpns(url):
